...
async def main():
    ...
    model = argv.pop(0) if len(argv) > 0 and argv[0] in ('claude', 'gpt', 'o3', 'gemini') else 'claude'
    if len(argv) < 1 or not os.path.exists(argv[0]):
        print("Usage: python main.py [claude|gpt|o3|gemini] <directory> [prompt]")
        sys.exit(1)
    ...
    if model == 'gpt':
        ...
        model = agents.OpenAIChatCompletionsModel("gemini-2.5-pro-preview-05-06", client)
    elif model == 'claude':
...
"""
    agent = agents.Agent("code", instructions, model, tools)
    messages = []
    while True:
